...
describe('10 - Implemente a função `createMenu`, bem como seus casos de teste', () => {
  it('Verifique se função `createMenu()` retorna um objeto que possui a chave `fetchMenu`, a qual tem como valor uma função.', () => {
    expect(typeof createMenu(obj).fetchMenu).toBe('function');
  });
  ...
  // ```
  // TESTE 2:Verifique se função `createMenu()` retorna um objeto que possui a chave `fetchMenu`, a qual tem como valor uma função..
  // ```
  ...
  it('Verifique se, ao adicionar três pedidos, dentre bebidas e comidas, o array `objetoRetornado.consumption` contém os itens pedidos', () => {
    const objetoRetornado = createMenu(obj);
    objetoRetornado.order('coxinha');
    objetoRetornado.order('hamburguer');
    objetoRetornado.order('feijoada');
    expect(objetoRetornado.consumption).toEqual(['coxinha','hamburguer','feijoada']);
  });
  ...
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    expect(objetoRetornado.pay).toBe(27.060000000000002);
  });
