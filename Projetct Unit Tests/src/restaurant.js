...
const objectExample = { food: { coxinha: 3.9, sopa: 9.9 }, drink: { agua: 3.9, cerveja: 6.9 } };

const pagamento = (obj) => {
  const pricesFood = Object.values(obj.food);
  const pricesDrink = Object.values(obj.drink);
  let valueFood = 0;
  let valueDrink = 0;
  for (let i = 0; i < pricesFood.length; i += 1) {
    valueFood += pricesFood[i];
  }
  for (let i = 0; i < pricesDrink.length; i += 1) {
    valueDrink += pricesDrink[i];
  }
  return 1.1 * (valueFood + valueDrink);
};

const createMenu = (objetoPassadoPorParametro) => {
  const obj = { fetchMenu: () => objetoPassadoPorParametro, 
  consumption: [],
  order: (string) => obj.consumption.push(string),
  pay: pagamento(objetoPassadoPorParametro), 
  };
